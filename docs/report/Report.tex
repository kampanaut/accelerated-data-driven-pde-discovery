...
\title{Meta-Learning for Accelerated PDE Discovery}
%
%
% author  
...
which can be extracted via Jacobian analysis.
...
transfer problem. Rather than learning a single set of weights for one system,
...
% NOTE: Also mention how Raissi did the experiment first, the coupled Solution Network and
% PDE Operator Network. What the loss function looks like, so that we can communicate more
% what the paper is trying to amortise.

In this paper MAML will be used to find the meta-parameter
\emph{initialisation} $\theta^{*}$ for PDE operator network
$\mathcal{N}_{\rho}$ for the PDE $\rho$. $\mathcal{N}_\rho$ is initialised with
with random $\theta_0$ parameters. $\tau(\rho)$ is the task distribution for
the $\rho$ family, and contains tasks of $\{\vec{x}_{(i)}, \vec{y}_{(i)}\}$
pairs. $\vec{x}_{(i)}$ is the input spatial derivatives features, where
$\vec{y_{(i)}}$ is the target time derivatives.

% TODO(me): Write 1-2 paragraphs here covering:
% - What this project specifically does: applies MAML to PDE operator networks
%   across multiple PDE families (NS, Brusselator, lambda-omega, heat, NL heat, FHN)
% - The research questions: Does MAML accelerate PDE discovery? Does it
%   generalise to OOD initial conditions? What determines successful
%   coefficient recovery?
% - Brief paper roadmap (Section II: related work, III: methodology, etc.)
