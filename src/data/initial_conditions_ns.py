...
def gaussian_hill_ic(
    ...
    psi = solve_poisson_2d(vorticity, x, y, periodic=True)
...
def shear_layer_ic(
    ...
    Creates two opposing velocity transitions at y = Ly/4 and y = 3Ly/4,
    forming a periodic jet profile compatible with Fourier spectral solvers.

    u(y) = (U/2) * [tanh((y - Ly/4) / δ) - tanh((y - 3Ly/4) / δ) - 1]

    Args:
        y_center: Unused (kept for config compatibility). Layer positions
            are fixed at Ly/4 and 3Ly/4 for periodicity.
        thickness: Thickness of transition regions
        velocity_jump: Velocity difference across each layer
    ...
    """
    X, Y = np.meshgrid(x, y)
    Lx = x[-1] + (x[1] - x[0])  # periodic domain length
    Ly = y[-1] + (y[1] - y[0])

    # Double shear layer: transitions at Ly/4 (up) and 3Ly/4 (down)
    # u ≈ -U/2 at edges, +U/2 in middle — periodic
    y1 = Ly / 4
    y2 = 3 * Ly / 4
    u_base = (velocity_jump / 2) * (
        np.tanh((Y - y1) / thickness) - np.tanh((Y - y2) / thickness) - 1
    )

    # Sinusoidal perturbation to trigger KH instability
    v_pert = perturbation_amplitude * np.sin(2 * np.pi * X / Lx)
...
def von_karman_street_ic(
    ...
    Returns:
        ...
        - vortex_params: List of dicts with actual vortex parameters
    """
    # domain_x = x[-1] - x[0] + (x[1] - x[0])
    domain_y = y[-1] - y[0] + (y[1] - y[0])
    center_y = domain_y / 2
