...
def generate_training_samples(
    ...
    print(
        f"  Targets: PDE RHS (D_A={D_A:.4f}, D_B={D_B:.4f}, k1={k1:.4f}, k2={k2:.4f})"
    )

    # Initialize training data dict
    # Using u/v naming to match NS for network compatibility
    # A → u, B → v
    column_size = len(concentration_history) * concentration_history[0][0].size
    training_data: DatasetGridFormat = {
        "u": np.zeros(column_size),  # A concentration
        "v": np.zeros(column_size),  # B concentration
        "u_x": np.zeros(column_size),  # A_x (first derivative - "dead" for Brusselator)
        "u_y": np.zeros(column_size),  # A_y (first derivative - "dead" for Brusselator)
        "u_xx": np.zeros(column_size),  # A_xx (active for Brusselator)
        "u_yy": np.zeros(column_size),  # A_yy (active for Brusselator)
        "v_x": np.zeros(column_size),  # B_x (first derivative - "dead" for Brusselator)
        "v_y": np.zeros(column_size),  # B_y (first derivative - "dead" for Brusselator)
        "v_xx": np.zeros(column_size),  # B_xx (active for Brusselator)
        "v_yy": np.zeros(column_size),  # B_yy (active for Brusselator)
        "u_t": np.zeros(column_size),  # A_t (from PDE RHS)
        "v_t": np.zeros(column_size),  # B_t (from PDE RHS)
    ...
    for i, (A, B) in enumerate(concentration_history):
        ...
        training_data["u"][start:end] = A.flatten()
        training_data["v"][start:end] = B.flatten()
        training_data["u_x"][start:end] = A_x.flatten()
        training_data["u_y"][start:end] = A_y.flatten()
        training_data["u_xx"][start:end] = A_xx.flatten()
        training_data["u_yy"][start:end] = A_yy.flatten()
        training_data["v_x"][start:end] = B_x.flatten()
        training_data["v_y"][start:end] = B_y.flatten()
        training_data["v_xx"][start:end] = B_xx.flatten()
        training_data["v_yy"][start:end] = B_yy.flatten()
        training_data["u_t"][start:end] = A_t.flatten()
        training_data["v_t"][start:end] = B_t.flatten()
    ...
    print(
        f"  ({n_snapshots} timesteps × {concentration_history[0][1].size} spatial points)"
    )
...
def save_metadata_txt(
    ...
    with open(txt_path, "w") as f:
        ...
        if success and training_data:
            ...
            f.write(
                "  Features: 10 (A, B, spatial derivatives - NS-compatible format)\n"
            )
            f.write("  Targets: 2 (A_t, B_t)\n")
            f.write("\n")
            ...
            for key, label in [("u", "A"), ("v", "B"), ("u_t", "A_t"), ("v_t", "B_t")]:
                if key in training_data:
                    ...
                    f.write(
                        f"  {label:4s}: [{values.min():>8.4f}, {values.max():>8.4f}]\n"
                    )
            f.write("\n")
...
def process_single_ic(args_tuple):
    ...
    *core_args, fourier_mode = args_tuple
    ic_config, simulation_params, data_dir, ic_idx, x, y = core_args
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            A_init, B_init, generated_params = create_brusselator_ic(
...
def main():
            ...
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    if args.fourier:
                        ...
                        print(
                            f"  A={training_data['u'][0]:.4f}, B={training_data['v'][0]:.4f}"
                        )
                        print(
                            f"  A_xx={training_data['u_xx'][0]:.4f}, A_yy={training_data['u_yy'][0]:.4f}"
                        )
                        print(
                            f"  A_t={training_data['u_t'][0]:.4f}, B_t={training_data['v_t'][0]:.4f}"
                        )
                        print(
    ...
    if successful > 0:
        ...
        print("  - Features: u(A), v(B), derivatives (10 inputs, NS-compatible format)")
        print("  - Targets: u_t(A_t), v_t(B_t) (2 outputs)")
