...
Solves u_t = D*nabla^2(u) for each IC, FFTs snapshots to Fourier
coefficients, saves .npz files.

Usage:
    python scripts/generate_heat_data.py --config configs/heat_train-1.yaml
    python scripts/generate_heat_data.py --config configs/heat_train-1.yaml --workers 4
"""
...
def save_heat_evolution(
    ...
    """
    Save a multi-panel figure showing heat equation evolution over time.
...
heat_spec = PDESpec(
    ...
    default_output_name="heat",
    extra_save_keys=[("D_used", "D")],
    save_visualization=save_heat_evolution,
)
