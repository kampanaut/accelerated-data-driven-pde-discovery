...
Solves FHN reaction-diffusion equations for each IC, FFTs snapshots to
Fourier coefficients, saves .npz files.

Usage:
    python scripts/generate_fhn_data.py --config configs/fhn_train-1.yaml
    python scripts/generate_fhn_data.py --config configs/fhn_train-1.yaml --workers 4
"""
...
def save_fhn_evolution(
    ...
    - Row 3: phase theta = atan2(v, u) (cyclic colormap)
...
fhn_spec = PDESpec(
    ...
    pde_param_keys=["D_u", "D_v", "eps", "a", "b"],
    samplable_params=["D_u", "D_v", "a"],
    create_ic=create_fhn_ic,
    solve=solve_fhn,
    field_type=FieldType.PAIRED,
    default_output_name="fhn",
    save_visualization=save_fhn_evolution,
)
