...
def load_model_from_checkpoint(checkpoint_path: Path, device: str, model_config: dict = None) -> PDEOperatorNetwork:
    """
    ...
    # Get architecture - prefer explicit config, then try checkpoint
    if model_config is None:
        saved_config = checkpoint.get('config', {})
        # Handle both dict (initial_model.pt) and MAMLConfig (best_model.pt)
        if isinstance(saved_config, dict):
            model_config = saved_config
        else:
            # MAMLConfig doesn't have model architecture, use defaults
            model_config = {}
        ...
        for noise_idx, noise in enumerate(noise_levels):
            ...
            try:
                ...
                maml_result = fine_tune(
                    maml_model, features, targets,
                    fine_tune_lr, max_steps, device,
                ...
                baseline_result = fine_tune(
                    baseline_model, features, targets,
                    fine_tune_lr, max_steps, device,
                    holdout_features=holdout_features,
                    holdout_targets=holdout_targets,
                )
                ...
                curves[f"{combo_key}/maml_train_losses"] = np.array(maml_result['train_losses'])
                curves[f"{combo_key}/maml_holdout_losses"] = np.array(maml_result['holdout_losses'])
...
def main():
    ...
    for mode_name, clean_targets in target_modes:
        ...
        for ic_type, entries in sorted(holdout_by_ic.items()):
            ...
            print(f"  {ic_type}: MAML={maml_mean:.2e}, Baseline={baseline_mean:.2e}{flag}")

            # List individual cases where MAML underperforms
            underperformers = [e for e in entries if e['maml_final'] > e['baseline_final']]
            if underperformers:
                print(f"    Underperforming cases ({len(underperformers)}/{len(entries)}):")
                for e in underperformers[:5]:  # Show at most 5
                    print(f"      {e['task']} K={e['k']} noise={e['noise']:.0%}: MAML={e['maml_final']:.2e} > Baseline={e['baseline_final']:.2e}")
    ...
    for mode_name, _ in target_modes:
        ...
        print(f"    - results.json (metadata)")
        print(f"    - curves/*.npz (loss curves)")
    print()
    print("Next steps:")
